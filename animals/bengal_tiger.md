# Animals -- Bengal Tiger

**status:** research-integrated
**last_reconciled:** 2026-08-22

**Latin:** *Panthera tigris tigris* | **Continent:** Asia | **IUCN (in-game):** Endangered | **Housing type:** Habitat

## Housing & environment

- **Land min / +per adult:** 705 m² / +111 m²
- **Water min:** 0 m²
- **Climb min:** 0 m²
- **Temperature range:** 12-42 degC
- **Biome:** Grassland, Tropical, Temperate
- **Terrain composition:** grass 10-30% / soil 20-90% / rock 0-50% / sand 0-20% / snow 0%
- **Barrier:** grade 3, 3 m minimum height `[planetzoohelper 2026-09-17]`
- **Guest tolerance:** Confident -- unbothered by guest proximity or crowding. `[planetzoohelper 2026-09-17]`

## Social, diet, enrichment

- **Social type / group:** Solitary except mother+cubs
- **Mixing compatibility:** None
- **Diet:** Processed Meat / Whole Carcass / Whole Fish
- **Enrichment sample:** Frozen Blood Pumpkin, Restraint Feeder, Scratching Post
- **Sex-ratio cap:** 1 males : 1 females (mixed group) `[planetzoohelper 2026-09-17]`

## Welfare gotcha

**Fights with Siberian Tiger if mixed.** [Confirmed: 2 sources -- the same "rank" system also shows Cheetah/Snow Leopard share a rank but do NOT fight, so shared rank alone doesn't predict conflict.] Tiger urine smells like buttered popcorn (shared aroma compound). Cleared a 1m jump barrier in the community's controlled jump-height test -- one of only five species that could (see `mechanics.md` Habitats & Enclosures).

_source: Planet Zoo Wiki (Fandom) + planetzoohelper.com + Steam Community "Habitat Husbandry Guide and Planner" (senginous) 2026-08-21 · capture: web_fetch · confidence: medium · enemy-tier: 0 · puzzle-tier: 0 · category: mainline · spoiler: none_

## Sources
- Planet Zoo Wiki species page [community-wiki]
- planetzoohelper.com [datamining]
