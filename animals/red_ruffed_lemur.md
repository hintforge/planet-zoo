# Animals -- Red Ruffed Lemur

**status:** research-integrated
**last_reconciled:** 2026-08-22

**Latin:** *Varecia rubra* | **Continent:** Africa (Madagascar) | **IUCN (in-game):** Critically Endangered | **Housing type:** Habitat (Walkabout-flagged)

## Housing & environment

- **Land min / +per adult:** 210 m² / +8 m²
- **Water min:** 0 m²
- **Climb min:** 20 m²
- **Temperature range:** 10-42 degC
- **Biome:** Tropical
- **Terrain composition:** grass 10-40% / soil 10-90% / rock 0-50% / sand 0-20% / snow 0%
- **Guest tolerance:** Neutral -- tolerates normal viewing but needs somewhere to retreat when crowds build. Workable walkthrough if a single guest gate caps how many get inside at once. `[planetzoohelper 2026-09-17]`

## Social, diet, enrichment

- **Social type / group:** Highly social, up to ~30; 3-30
- **Mixing compatibility:** Black-and-White Ruffed Lemur, Coquerel's Sifaka (DLC), Ring Tailed Lemur
- **Diet:** Monkey Chow / Leaves & Shoots / Fruit
- **Enrichment sample:** Forage Box, Climbing Frame, Mirror Mobile

## Welfare gotcha

Grooms with a specialized claw on the second hind toe. Cited release value ~45 CC -- a low-mid CC example despite Critically Endangered status, illustrating that IUCN tier alone doesn't set payout (see `mechanics.md` Economy & Currencies).

_source: Planet Zoo Wiki (Fandom) + planetzoohelper.com + Steam Community "Habitat Husbandry Guide and Planner" (senginous) 2026-08-21 · capture: web_fetch · confidence: medium · enemy-tier: 0 · puzzle-tier: 0 · category: mainline · spoiler: none_

## Sources
- Planet Zoo Wiki species page [community-wiki]
- planetzoohelper.com [datamining]
