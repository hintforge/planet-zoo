# Animals -- Pygmy Hippopotamus

**status:** research-integrated
**last_reconciled:** 2026-08-22

**Continent:** Africa (Sierra Leone, Liberia, Guinea, Ivory Coast) | **IUCN (in-game):** Endangered | **Housing type:** Habitat

## Housing & environment

- **Land (individual / family):** ~540 m² / 660 m² -- source notes an equal land/water split, i.e. water area matches the land figures above
- **Water:** equal to land (per source note, not separately itemized)
- **Climb min:** 0 m² `[planetzoohelper 2026-09-17]`
- **Temperature range:** 12-42 degC
- **Biome(s):** Aquatic, Tropical
- **Terrain composition:** grass 0-50% / soil 20-60% / rock 20-40% / sand 0-40% / snow 0-0% `[planetzoohelper 2026-09-17]`
- **Barrier:** grade 3, 1 m minimum height `[planetzoohelper 2026-09-17]`
- **Guest tolerance:** Confident -- unbothered by guest proximity or crowding. `[planetzoohelper 2026-09-17]`

## Social, diet, enrichment

- **Social type / group:** 2-4 (1M, up to 3F)
- **Mixing compatibility:** Red River Hog
- **Diet/enrichment:** not specified in source data
- **Sex-ratio cap:** 1 males : 3 females (mixed group) `[planetzoohelper 2026-09-17]`

## Welfare gotcha

Mixes with Red River Hog -- the only Deluxe Upgrade Pack species with a listed compatible partner. One of only 3 species in the entire pack, a paid-upgrade DLC with no scenery content, purchasable by anyone regardless of purchase timing (not a console-exclusive pre-order lock on PC/Steam).

_source: Planet Zoo DLC research pass (P3) 2026-08-22 · capture: web_fetch · confidence: medium · enemy-tier: 0 · puzzle-tier: 0 · category: mainline · spoiler: dlc:Deluxe Upgrade Pack_

## Sources
- Steam store page (Deluxe Upgrade Pack, app 1098120) [official]
- Steam Community guide (filedetails id 3343060169) [community-guide]
- Villanelle's Data Chest spreadsheet [community-datamining]
