# Animals -- American Bison

**status:** research-integrated
**last_reconciled:** 2026-08-22

**Latin:** *Bison bison bison* | **Continent:** North America | **IUCN (in-game):** Near Threatened | **Housing type:** Habitat

## Housing & environment

- **Land:** 1,000 m² min, 100% land
- **Terrain:** 60% grass
- **Temperature range:** -20 to 30 degC (ideal 15)
- **Biome:** Grassland
- **Guest tolerance:** Neutral -- tolerates normal viewing but needs somewhere to retreat when crowds build. `[planetzoohelper 2026-09-17]`

## Social, diet, enrichment

- **Social type / group:** Herd, 15-400
- **Mixing compatibility:** Bighorn Sheep (DLC), Black-Tailed Prairie Dog (DLC), Collared Peccary, Pronghorn Antelope
- **Diet/enrichment:** Grazing Ball Feeder / Mud Bath / Rubbing Pillar / Ice Block

## Welfare gotcha

Poor eyesight but excellent hearing/smell; Zoopedia marks them "neutral to humans." [Confirmed: 2 sources -- corroborated by a Reddit thread on the Zoopedia trait]

_source: Fandom `Category:Base_Game_Animals`/`List_of_Animals` + `Category:Animals_by_Continent` + senginous Steam guide + Reddit Bison thread 2026-08-21 · capture: web_fetch · confidence: medium · enemy-tier: 0 · puzzle-tier: 0 · category: mainline · spoiler: none_

## Sources
- Fandom Category:Base_Game_Animals [community-wiki]
- Reddit r/PlanetZoo Bison thread [forum]
