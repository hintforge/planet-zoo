...
- **Stress is guest-visibility- and privacy-driven**: an animal takes a stress penalty when it wants to hide from guests and has no hard-shelter/retreat option; "more confident" species tolerate larger guest crowds before stress accrues than shy species at the same density. **One-way glass zeroes the guest-visibility stress term specifically** -- the animal's AI does not register being observed through it -- a targeted fix, not a general welfare boost. **The stat that sets this per species is Guest tolerance** (`Shy` / `Neutral` / `Confident`, the Zoopedia's relationship-with-humans line), recorded on every Habitat species in `animals/`: `Shy` needs hiding cover, one-way glass, or a low-traffic placement; `Neutral` needs somewhere to retreat when crowds build; `Confident` is unbothered. **This is the stat that decides whether a walkthrough habitat is a good idea** -- the Walkabout flag only decides whether the game will let you build one. Ten of the 31 Walkabout-eligible species are `Shy`, and a busy walkthrough will run them into low-welfare warnings; see `animals/index.md`.
